...
Graph tại `data/knowledge/pm25_knowledge_graph.json` cung cấp ba nhóm quan hệ có
nguồn tham chiếu:

- `EMITS`: phương tiện, nhà máy và xây dựng liên kết với các chất có thể phát thải
  (`PM2.5`, `PM10`, `NO2`, `SO2`, `CO`).
- `INFLUENCED_BY`: PM2.5 liên kết với gió, độ ẩm, mưa và nhiệt độ.
...
```powershell
Invoke-RestMethod -Uri "http://127.0.0.1:8000/knowledge-graph/pm25?relation=EMITS"
```
